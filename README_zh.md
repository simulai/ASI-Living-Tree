...
[M-Flow](https://github.com/FlowElement-ai/m_flow) 是一种领先的 Graph-RAG 方法，使用 4 级锥形图层次结构（Episode → Facet → FacetPoint → Entity）和证据路径评分。它在 LoCoMo-10 上达到 **81.8%**，在 LongMemEval 上达到 **89%**。

Living Tree 和 M-Flow 采用互补的架构方法：

| 方面 | M-Flow | ASI-Living-Tree |
|------|--------|-----------------|
| **核心机制** | 基于路径代价优化的图传播 | Hopfield 能量动力学 |
| **知识结构** | 4 级锥形图（Episode → Facet → FacetPoint → Entity） | 带元数据聚类的孤立节点 |
| **检索哲学** | "向量找候选，图决定相关" | 能量景观决定赢家 |
| **歧义处理** | 通过锥形层次进行多粒度路由 | 聚类级能量间隔检测 |
| **更新成本** | O(N) - 受图结构影响 | O(1) - 节点隔离 |
| **遗忘** | 取决于实现 | 零 - 节点隔离 |
| **主要优势** | 跨文档实体桥接、语义路径推理 | 代码版本冲突、确定性检索 |
| **目标领域** | 通用记忆/聊天机器人 | 代码 RAG、API 版本路由 |

**关键洞察**：M-Flow 擅长通过图遍历找到连贯的 Episode bundle。Living Tree 擅长在**冲突版本之间进行确定性路由**（如 API v1 vs v2），具有 258x 的区分度。这些是互补的能力——未来的系统可以结合 M-Flow 的图推理与 Living Tree 的能量间隔路由。
